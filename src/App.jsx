...
const BIOME_TIME = 24,
  BIOME_TRANS = 3.5; // durée d'un biome (s) et fondu
const pickBiomeExcept = (cur) => {
  let i;
  do {
    i = (Math.random() * BIOMES.length) | 0;
  } while (i === cur);
  return i;
};
...
const COIN_SHIELD = 1,
  COIN_BOXE = 1; // 1 pièce par voiture cassée (bouclier comme gant)
...
const SHAPES = [
  // SKINS = formes de voiture (s'achètent)
...
function loadSave() {
  try {
    ...
    if (r && typeof r.coins === 'number') return { ...DEFAULT_SAVE, ...r };
  } catch (e) {}
  return { ...DEFAULT_SAVE };
}
...
function createAudio() {
  ...
  const stopSiren = () => {
    ...
    const t = now();
    try {
      sirenNode.g.gain.cancelScheduledValues(t);
      sirenNode.g.gain.setTargetAtTime(0.0001, t, 0.06);
      sirenNode.o.stop(t + 0.25);
    } catch (e) {}
    sirenNode = null;
  };
  const startSiren = () => {
    // wail deux tons sur ~8,5 s
    stopSiren();
    const t = now();
    const o = ctx.createOscillator();
    o.type = 'sawtooth';
    ...
    let tt = t;
    o.frequency.setValueAtTime(600, tt);
    for (let i = 0; i < 21; i++) {
      o.frequency.linearRampToValueAtTime(i % 2 ? 600 : 920, tt + 0.4);
      tt += 0.4;
    } // ~8,4 s
    g.gain.setValueAtTime(0.1, tt - 0.15);
    g.gain.exponentialRampToValueAtTime(0.0001, tt);
    o.connect(g);
    g.connect(master);
    o.start(t);
    o.stop(tt + 0.05);
  ...
  return {
    ...
    engineOn(on) {
      eGain.gain.setTargetAtTime(on ? 0.06 : 0, now(), 0.08);
    },
    setEngine(level) {
      eOsc.frequency.setTargetAtTime(62 + level * 16, now(), 0.2);
    },
    move() {
      tone('square', 660, 1040, 0.1, 0.05);
    }, // petit tic gauche/droite
    crash() {
      burst(700, 0.6, 0.6, 0.28);
      tone('sine', 130, 40, 0.5, 0.25);
    },
    explosion() {
      burst(1300, 0.7, 0.5, 0.35);
      tone('square', 600, 70, 0.18, 0.3);
    },
    pickup() {
      tone('square', 520, 1040, 0.25, 0.16);
    },
    swing() {
      burst(1800, 0.4, 0.22, 0.12);
    }, // gant qui jaillit
    beep() {
      tone('square', 620, 620, 0.3, 0.16);
    }, // 3, 2, 1 : trois fois le MÊME son
    sirenStart() {
      startSiren();
    }, // sirène : son tenu pendant les 8 s
    sirenStop() {
      stopSiren();
    },
    go() {
      // GO : un son tenu, continu
      const t = now(),
        dur = 0.85;
      const o = ctx.createOscillator();
      o.type = 'sawtooth';
    ...
    punchHit() {
      burst(500, 0.6, 0.45, 0.16);
      tone('square', 200, 55, 0.4, 0.2);
    },
    defeat() {
      const t = now();
      [440, 370, 294, 196].forEach((f, i) =>
        tone('triangle', f, f, 0.3, 0.2, t + i * 0.18)
      );
    },
  };
}
...
function drawDecorItem(ctx, type, cx, baseY, h) {
  if (type === 'tree') {
    ...
    ctx.fillRect(cx - 2, baseY - 10, 4, 12);
    ctx.fillStyle = h > 0.5 ? '#2f7d3a' : '#3a9a4a';
    ctx.beginPath();
    ctx.arc(cx, baseY - 16, 9 + h * 3, 0, 7);
    ctx.fill();
    ...
    const bw = 18 + h * 9,
      ...
      x = cx - bw / 2,
      top = baseY - bh;
    ctx.fillStyle = ['#8a93a3', '#74809a', '#9aa3b3'][(h * 3) | 0];
    ...
    if (h > 0.4) {
      ctx.fillRect(cx - 9, baseY - 14, 6, 4);
      ctx.fillRect(cx - 9, baseY - 18, 4, 6);
    }
    if (h < 0.6) {
      ctx.fillRect(cx + 3, baseY - 20, 6, 4);
      ctx.fillRect(cx + 5, baseY - 24, 4, 6);
    }
  } else if (type === 'light') {
    const g = ctx.createRadialGradient(cx, baseY - 10, 1, cx, baseY - 10, 12);
    g.addColorStop(0, 'rgba(255,225,140,.95)');
    g.addColorStop(1, 'rgba(255,200,80,0)');
    ctx.fillStyle = g;
    ctx.beginPath();
    ctx.arc(cx, baseY - 10, 12, 0, 7);
    ctx.fill();
    ctx.fillStyle = '#ffe08a';
    ctx.beginPath();
    ctx.arc(cx, baseY - 10, 3, 0, 7);
    ctx.fill();
  } else if (type === 'star') {
    ctx.fillStyle = `rgba(255,255,255,${0.35 + 0.6 * h})`;
    ctx.fillRect(cx - 6, baseY - 34, 2, 2);
    ctx.fillRect(cx + 5, baseY - 14, 2, 2);
...
const SHAPE_DRAW = {
  berline(ctx, color) {
    ctx.fillStyle = color;
    rr(ctx, -25, -43, 50, 86, 12);
    ctx.fill();
    ctx.fillStyle = '#1c1c22';
    ctx.fillRect(-28, -31, 5, 18);
    ctx.fillRect(-28, 13, 5, 18);
    ctx.fillRect(23, -31, 5, 18);
    ctx.fillRect(23, 13, 5, 18);
    ctx.fillStyle = 'rgba(255,255,255,.55)';
    ctx.fillRect(-16, -31, 32, 18);
  ...
  sport(ctx, color) {
    ctx.fillStyle = '#1c1c22';
    ctx.fillRect(-24, -22, 5, 15);
    ctx.fillRect(-24, 12, 5, 16);
    ctx.fillRect(19, -22, 5, 15);
    ctx.fillRect(19, 12, 5, 16);
    ctx.fillStyle = color;
    ctx.beginPath();
    ctx.moveTo(0, -44);
    ctx.lineTo(19, -18);
    ...
    ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,.55)';
    rr(ctx, -12, -14, 24, 20, 7);
    ctx.fill();
    ctx.fillStyle = '#1c1c22';
    ctx.fillRect(-20, 38, 40, 5); // aileron
    ctx.fillStyle = '#fff7cc';
    ctx.beginPath();
    ctx.arc(-8, -38, 2.5, 0, 7);
    ctx.fill();
    ctx.beginPath();
    ctx.arc(8, -38, 2.5, 0, 7);
    ctx.fill();
  ...
  police(ctx, color) {
    ...
    ctx.fillRect(-25, -3, 50, 9); // bande blanche
    ctx.fillStyle = '#e11d2a';
    ctx.fillRect(-11, -5, 11, 7); // gyrophare rouge
    ctx.fillStyle = '#1763ff';
    ctx.fillRect(0, -5, 11, 7); // gyrophare bleu
  },
  tracteur(ctx, color) {
    ...
    ctx.fill();
    rr(ctx, 14, 6, 13, 32, 5);
    ctx.fill(); // grosses roues arrière
    rr(ctx, -22, -34, 8, 16, 3);
    ctx.fill();
    rr(ctx, 14, -34, 8, 16, 3);
    ctx.fill(); // petites roues avant
    ctx.fillStyle = color;
    rr(ctx, -16, -36, 32, 72, 8);
    ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,.85)';
    rr(ctx, -12, -30, 24, 10, 3);
    ctx.fill(); // calandre
    ctx.fillStyle = 'rgba(255,255,255,.5)';
    rr(ctx, -13, 4, 26, 22, 5);
    ctx.fill(); // cabine
    ctx.fillStyle = '#444';
    ctx.fillRect(-3, -42, 6, 9); // pot d'échappement
  },
  monster(ctx, color) {
    ctx.fillStyle = '#15151a';
    const wheel = (x, y) => {
      rr(ctx, x - 9, y - 15, 18, 30, 6);
      ctx.fill();
      ctx.fillStyle = '#39394a';
      ctx.beginPath();
      ctx.arc(x, y, 5, 0, 7);
      ctx.fill();
      ctx.fillStyle = '#15151a';
    ...
    wheel(22, 25);
    ctx.fillStyle = color;
    rr(ctx, -18, -40, 36, 80, 10);
    ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,.55)';
    rr(ctx, -12, -32, 24, 17, 5);
    ctx.fill(); // cabine
  ...
  f1(ctx, color) {
    ...
    ctx.fill();
    rr(ctx, 12, -30, 9, 17, 3);
    ctx.fill(); // roues avant
    rr(ctx, -22, 14, 10, 18, 3);
    ctx.fill();
    rr(ctx, 12, 14, 10, 18, 3);
    ctx.fill(); // roues arrière
    ctx.fillStyle = color;
    rr(ctx, -23, -44, 46, 7, 2);
    ctx.fill(); // aileron avant
    rr(ctx, -20, 36, 40, 8, 2);
    ctx.fill(); // aileron arrière
    ctx.beginPath();
    ctx.moveTo(0, -41);
    ctx.lineTo(8, -18);
    ctx.lineTo(9, 32);
...
function drawSirenIcon(ctx, cx, cy, sc) {
  ...
  rays.forEach((p, i) => {
    ...
    ctx.beginPath();
    ctx.moveTo(p[0], p[1]);
    ctx.lineTo(p[0] + Math.cos(a) * 5, p[1] + Math.sin(a) * 5);
    ctx.stroke();
  });
  ...
  ctx.save();
  ctx.beginPath();
  ctx.rect(-R - 1, by - R - 1, R + 1, R * 2 + 2);
  ctx.clip();
  ctx.beginPath();
  ctx.arc(0, by, R, Math.PI, 0, true);
  ctx.closePath();
...
function drawBonus(ctx, b, t) {
  ...
  const styles = {
    shield: {
      glow: 'rgba(120,200,255,.9)',
      glow0: 'rgba(70,140,255,0)',
      disc: '#2f86ff',
      ring: '#dff0ff',
    },
    retreci: {
      glow: 'rgba(150,240,160,.95)',
      glow0: 'rgba(60,180,90,0)',
      disc: '#2ec45f',
      ring: '#e3ffe9',
    },
    boxe: {
      glow: 'rgba(255,180,120,.95)',
      glow0: 'rgba(230,90,40,0)',
      disc: '#ff7a33',
      ring: '#ffe3cf',
    },
    sirene: {
      glow: 'rgba(255,200,200,.95)',
      glow0: 'rgba(225,30,30,0)',
      disc: '#ffffff',
      ring: '#e11d2a',
    },
  };
  ...
  grad.addColorStop(1, st.glow0);
  ctx.fillStyle = grad;
  ctx.beginPath();
  ctx.arc(b.x, b.y, r + 8, 0, 7);
  ctx.fill();
  ctx.fillStyle = st.disc;
  ctx.beginPath();
...
function ShapePreview({ shape, color, size = 42 }) {
  ...
  useEffect(() => {
    ...
    const Wp = size,
      Hp = Math.round(size * 1.35);
    c.width = Wp * dpr;
    c.height = Hp * dpr;
    const x = c.getContext('2d');
    x.scale(dpr, dpr);
    x.clearRect(0, 0, Wp, Hp);
    const sc = Math.min(Wp / 60, Hp / 100);
    x.save();
    x.translate(Wp / 2, Hp / 2);
    x.scale(sc, sc);
    drawVehicle(x, 0, 0, color, -1, shape);
    x.restore();
  }, [shape, color, size]);
  return (
    <canvas
      ref={ref}
      style={{ width: size, height: Math.round(size * 1.35), display: 'block' }}
    />
  );
}
...
function CarIcon({ alive }) {
  ...
  return (
    <svg width="26" height="32" viewBox="0 0 26 32" aria-hidden="true">
      ...
      />
      {!alive && (
        <path d="M6 8 L20 24 M20 8 L6 24" stroke="#ffd23f" strokeWidth="2.5" />
      )}
    </svg>
  );
}
function GloveBtnIcon({ color = '#fff' }) {
  return (
    <svg width="30" height="30" viewBox="0 0 32 32" aria-hidden="true">
      <g fill={color}>
        <rect x="9" y="22" width="14" height="7" rx="3" />
        <rect x="7" y="6" width="18" height="18" rx="9" />
...
export default function App() {
  ...
  saveRef.current = save; // miroir toujours à jour pour la boucle
  ...
  const commitSave = useCallback((updater) => {
    setSave((prev) => {
  ...
  const triggerSiren = useCallback(() => {
    ...
    s.siren = 8;
    s.sirenReady = false; // ouvre la voie pendant 8 s
    s.sirenLane = clamp(
      Math.round((s.playerX - SHOULDER - LANE_W / 2) / LANE_W),
      0,
      LANES - 1
    ); // la voie occupée à l'activation
    audioRef.current?.sirenStart();
  }, []);

  const launchGame = useCallback(() => {
    // prépare une partie et lance le décompte
    ...
    g.current = {
      ...
      biomePeriod: 0,
      biomeIdx: b0,
      biomeNext: pickBiomeExcept(b0),
      biomeFlash: 0,
      bg: {
        ground: BIOMES[b0].ground,
        road: BIOMES[b0].road,
        dash: BIOMES[b0].dash,
        edge: BIOMES[b0].edge,
        dark: BIOMES[b0].dark,
        decorA: BIOMES[b0].decor,
        decorB: BIOMES[b0].decor,
        blend: 0,
        name: BIOMES[b0].name,
      },
      cdN: 3,
      cdT: 0, // compte à rebours 3-2-1-GO
  ...
  const beginDuel = useCallback(() => {
    // crée le duel et passe au 1er tour
    const a = (duelNames[0] || '').trim().slice(0, 14),
      b = (duelNames[1] || '').trim().slice(0, 14);
    if (!a || !b) return;
    setTour({
      mode: 'duel',
      gamesPer: duelGames,
  ...
  const startTurn = useCallback(() => {
    // lance la partie du joueur dont c'est le tour
    const t = tour;
    if (!t) return;
  ...
  const afterDuelGame = useCallback(() => {
    // enregistre le score et décide de la suite
    setTour((t) => {
      if (!t || t.mode !== 'duel') return t;
      const scores = [t.scores[0].slice(), t.scores[1].slice()];
      scores[t.turn].push(lastScore);
      const finished =
        scores[0].length >= t.gamesPer && scores[1].length >= t.gamesPer;
      if (t.turn === 0) {
        setMode('turnIntro');
        return { ...t, scores, turn: 1 };
      } // au tour du joueur 2
  ...
  const resumeRun = useCallback(() => {
    // sortie du garage : on reprend la route
  ...
  const chooseColor = (hex) => {
    // la couleur est GRATUITE
    colorRef.current = hex;
    commitSave((p) => ({ ...p, color: hex }));
  };
  const buyProtection = (id) => {
    const pr = PROTECTIONS.find((x) => x.id === id),
      cur = saveRef.current,
      s = g.current;
  ...
  useEffect(() => {
    const down = (e) => {
      if (
        ...
        return; // on tape son nom : ne pas intercepter
      if (
      ...
      if (e.key === 'ArrowLeft' || e.key === 'a') {
        if (!s.keys.left && playing) audioRef.current?.move();
        s.keys.left = true;
      }
      if (e.key === 'ArrowRight' || e.key === 'd') {
        if (!s.keys.right && playing) audioRef.current?.move();
        s.keys.right = true;
      }
  ...
  useEffect(() => {
    ...
    const carNearTop = (s, cx) =>
      s.cars.some((c) => Math.abs(c.x - cx) < 8 && c.y < 150);

    const spawnBand = (s) => {
      const cands = [0, 1, 2, 3].filter(
        (l) =>
          l !== s.gapLane &&
          !itemNearTop(s, laneCenter(l)) &&
          !(s.siren > 0 && l === s.sirenLane)
      );
      ...
      for (const lane of cands.slice(0, CARS_PER_BAND[s.level])) {
        ...
        s.cars.push({
          x: laneCenter(lane),
          y: -CAR_H,
          color: black ? '#17171c' : pick(PALETTE),
          black,
        });
      }
    ...
    const loop = (t) => {
      ...
      if (s && statusRef.current === 'countdown') {
        s.cdT += dt;
        if (s.cdN > 0 && s.cdT >= 0.7) {
          s.cdT = 0;
          s.cdN -= 1;
          if (s.cdN >= 1) a?.beep();
          else a?.go();
        } else if (s.cdN === 0 && s.cdT >= 0.7) {
          s.cdT = 0;
          s.elapsed = 0;
          setMode('playing');
        }
      ...
      if (playing) {
        ...
        const blend =
          into > BIOME_TIME - BIOME_TRANS
            ? (into - (BIOME_TIME - BIOME_TRANS)) / BIOME_TRANS
            : 0;
        const BA = BIOMES[s.biomeIdx],
          BB = BIOMES[s.biomeNext];
        s.bg = {
          ground: lerpColor(BA.ground, BB.ground, blend),
          road: lerpColor(BA.road, BB.road, blend),
          dash: lerpColor(BA.dash, BB.dash, blend),
          edge: lerpColor(BA.edge, BB.edge, blend),
          dark: BA.dark + (BB.dark - BA.dark) * blend,
          decorA: BA.decor,
          decorB: BB.decor,
          blend,
          name: BA.name,
        ...
        if (s.bonusTimer <= 0) {
          ...
          const free = [s.gapLane, 0, 1, 2, 3].find(
            (l) => !carNearTop(s, laneCenter(l))
          );
          if (free != null) {
            s.bonuses.push({
              x: laneCenter(free),
              y: -30,
              type: pick(['boxe', 'shield', 'retreci', 'sirene']),
            });
            s.bonusTimer = rand(BONUS_EVERY[0], BONUS_EVERY[1]);
          } else {
            s.bonusTimer = 0.4;
          } // tout est occupé : on réessaie vite
        }
        for (const b of s.bonuses) b.y += descent * dt;
        for (const b of s.bonuses) {
          if (
            Math.abs(b.x - s.playerX) < CAR_W / 2 + 14 &&
            Math.abs(b.y - PLAYER_Y) < CAR_H / 2 + 14
          ) {
            if (b.type === 'shield') {
              s.shieldArmed = true;
              s.shield = 0;
            } // chrono démarrera au 1er choc
            else if (b.type === 'retreci') s.shrink = 10;
            else if (b.type === 'sirene')
              s.sirenReady = true; // s'active à la flèche bas
        ...
        if (s.garageTimer <= 0 && !s.garage) {
          const free = [s.gapLane, 0, 1, 2, 3].find(
            (l) =>
              !carNearTop(s, laneCenter(l)) && !itemNearTop(s, laneCenter(l))
          );
          if (free != null) {
            s.garage = { x: laneCenter(free), y: -60 };
            s.garageTimer = rand(40, 55);
          } else s.garageTimer = 0.4;
        }
        if (s.garage) {
          s.garage.y += descent * dt;
          if (
            Math.abs(s.garage.x - s.playerX) < LANE_W / 2 &&
            Math.abs(s.garage.y - PLAYER_Y) < CAR_H / 2 + 22
          ) {
            setSave((p) => {
              const next = { ...p, coins: walletRef.current };
              writeSave(next);
              return next;
            }); // on encaisse
            s.cars = [];
            ...
            s.garage = null;
            s.siren = 0;
            s.distSinceBand = BAND_GAP[s.level];
            s.bonusTimer = rand(BONUS_EVERY[0], BONUS_EVERY[1]);
            a?.engineOn(false);
            a?.sirenStop();
            a?.pickup();
        ...
        if (s.punch) {
          ...
          const ext =
            PUNCH_REACH * Math.sin(Math.min(1, s.punch.prog) * Math.PI);
          const tipY = frontY - 16 - ext;
          if (!s.punch.connected) {
            for (const c of s.cars) {
              if (c.y > H + 999 || c.black) continue; // une voiture noire ne se casse pas
              if (
                Math.abs(c.x - s.playerX) < CAR_W / 2 + 8 &&
                tipY > c.y - CAR_H / 2 - 8 &&
                tipY < c.y + CAR_H / 2 + 8
              ) {
                s.punch.connected = true;
                explode(s, c.x, c.y, c.color);
                c.y = H + 999;
                addPts(s, c.x, c.y, PTS_BOXE, COIN_BOXE);
                a?.punchHit();
                break;
              }
            }
        ...
        for (const c of s.cars) {
          ...
          if (sirenSafe !== null && Math.abs(c.x - sirenSafe) < LANE_W * 0.55)
            continue; // voie ouverte par la sirène
          if (
            Math.abs(c.x - s.playerX) < (playerW + CAR_W) / 2 - pad &&
            Math.abs(c.y - PLAYER_Y) < (playerH + CAR_H) / 2 - pad
          ) {
            if (c.black) {
              // VOITURE NOIRE : indestructible et dangereuse
              if (s.invuln <= 0) {
                s.shield = 0;
                s.shieldArmed = false;
                s.punches = 0;
                s.shrink = 0; // casse TOUS les bonus
                s.lives -= 1;
                c.y = H + 999;
                explode(s, c.x, c.y, '#17171c');
                if (s.lives <= 0) {
                  const fscore = Math.floor(s.score);
                  setLastScore(fscore);
                  setSave((p) => {
                    const next = {
                      ...p,
                      coins: walletRef.current,
                      best: Math.max(p.best, fscore),
                    };
                    writeSave(next);
                    return next;
                  });
            ...
            if (s.shield > 0 || s.shieldArmed) {
              // protégé : la voiture explose
              if (s.shieldArmed && s.shield <= 0) s.shield = SHIELD_TIME; // <-- le chrono démarre ICI
              explode(s, c.x, c.y, c.color);
              c.y = H + 999;
              addPts(s, c.x, c.y, PTS_SHIELD, COIN_SHIELD);
              a?.explosion();
            } else if (s.invuln <= 0) {
              // touché : on perd une vie
              s.lives -= 1;
              c.y = H + 999;
              explode(s, c.x, c.y, '#e63946');
              if (s.lives <= 0) {
                const fscore = Math.floor(s.score);
                setLastScore(fscore);
                setSave((p) => {
        ...
        if (s.hudT > 0.1) {
          ...
          setHud({
            time: `${m}:${sec}`,
            speed: SPEEDS_KMH[s.level],
            lives: s.lives,
            score: Math.floor(s.score),
            shield: s.shield > 0 ? Math.ceil(s.shield) : s.shieldArmed ? -1 : 0,
            punches: s.punches,
            shrink: s.shrink > 0 ? Math.ceil(s.shrink) : 0,
            siren: s.siren > 0 ? Math.ceil(s.siren) : s.sirenReady ? -1 : 0,
            coins: Math.floor(walletRef.current),
          });
        }
      }
      ...
      if (s) {
        // décor des bas-côtés (se fond pendant les transitions)
        drawDecor(ctx, fb.decorA, s.worldDist, 1 - fb.blend);
        if (fb.blend > 0) drawDecor(ctx, fb.decorB, s.worldDist, fb.blend);
      }
      ctx.fillStyle = fb.road;
      ctx.fillRect(SHOULDER, 0, ROAD_W, H);
      const off = s ? s.scroll : 0;
      ctx.fillStyle = fb.dash;
      for (let l = 1; l < LANES; l++) {
        const x = SHOULDER + l * LANE_W - 2;
        for (let y = -40 + off; y < H; y += 40) ctx.fillRect(x, y, 4, 22);
      }
      ctx.fillStyle = fb.edge;
      ctx.fillRect(SHOULDER - 3, 0, 3, H);
      ctx.fillRect(W - SHOULDER, 0, 3, H);
      ...
      if (s) {
        for (const c of s.cars) {
          ...
          if (c.black) {
            // contour rouge "danger" qui pulse
            ctx.strokeStyle = `rgba(255,45,45,${
              0.55 + 0.4 * Math.sin(t / 110)
            })`;
            ctx.lineWidth = 3;
            rr(
              ctx,
        ...
        if (s.garage && s.garage.y < H + 60) {
          // panneau de sortie "GARAGE"
          const gx = s.garage.x,
            gy = s.garage.y,
            gw = LANE_W - 8,
            gh = 54;
          ctx.fillStyle = '#1f9e8a';
          rr(ctx, gx - gw / 2, gy - gh / 2, gw, gh, 10);
          ctx.fill();
          ctx.fillStyle = '#0d6b5d';
          rr(ctx, gx - gw / 2, gy - gh / 2, gw, 18, 10);
          ctx.fill();
          ctx.fillStyle = '#fff';
          ctx.font = 'bold 12px system-ui';
          ctx.textAlign = 'center';
          ctx.textBaseline = 'middle';
          ctx.fillText('GARAGE', gx, gy - 4);
        ...
        if (
          ...
          if (s.shieldArmed || s.shield > 0) {
            ...
            ctx.lineWidth = 4;
            ctx.strokeStyle = '#43a6ff';
            ctx.beginPath();
            ctx.arc(s.playerX, PLAYER_Y, rad, 0, 7);
            ctx.stroke();
            ctx.globalAlpha = blink * 0.18;
          ...
          if (s.siren > 0) {
            ...
            for (let i = 0; i < 2; i++) {
              ...
              ctx.globalAlpha = Math.max(0, 0.5 - rad / 110);
              ctx.lineWidth = 3;
              ctx.strokeStyle = col;
              ctx.beginPath();
              ctx.arc(s.playerX, PLAYER_Y, rad, 0, 7);
            ...
            const lw = 18 * pscale,
              lh = 8 * pscale;
            ctx.fillStyle = col;
            rr(ctx, s.playerX - lw / 2, PLAYER_Y - lh / 2 - 3, lw, lh, 3);
            ctx.fill();
            ctx.fillStyle = ph ? '#1e6bff' : '#e11d2a';
            rr(ctx, s.playerX - lw / 2, PLAYER_Y - lh / 2 - 3, lw / 2, lh, 3);
            ctx.fill();
          }
        }
        for (const p of s.popups) {
        ...
        if (fb.dark > 0) {
          ctx.fillStyle = `rgba(0,0,0,${fb.dark})`;
          ctx.fillRect(0, 0, W, H);
        }
        ...
        if (statusRef.current === 'countdown') {
          ...
          if (activeNameRef.current) {
            ...
            ctx.textBaseline = 'middle';
            ctx.fillText(
              'Au tour de ' + activeNameRef.current,
              W / 2,
              H / 2 - 90
            );
          }
  ...
  const press = (side, v) => (e) => {
    ...
    const s = g.current;
    if (!s) return;
    if (v && !s.keys[side] && statusRef.current === 'playing')
  ...
  const duelTotals = (t) => [
    t.scores[0].reduce((a, b) => a + b, 0),
    t.scores[1].reduce((a, b) => a + b, 0),
  ];
  const DuelTable = (t) => {
    ...
    const head = { ...cell, fontSize: 10, opacity: 0.6, letterSpacing: 0.5 };
              ...
              <div key={p} style={{ display: 'contents' }}>
                ...
                {Array.from({ length: rounds }).map((_, i) => (
                  <div
                    ...
                    style={{
                      ...cell,
                      opacity: t.scores[p][i] == null ? 0.35 : 1,
                    }}
                  >
  ...
  return (
    <div style={wrap}>
      <h1
        style={{ margin: 0, fontSize: 28, fontWeight: 900, letterSpacing: 1 }}
      >
        MONSTER DODGE <span style={{ fontSize: 20 }}>🛻</span>
      </h1>
      ...
      <div style={stage}>
        ...
        {status === 'playing' && (
          <>
            <div style={{ position: 'absolute', top: 10, left: 10, ...pill }}>
              <div style={label}>TEMPS</div>
              {hud.time}
            </div>
            <div
              style={{
            ...
            >
              <div style={label}>VITESSE</div>
              {hud.speed}
              <span style={{ fontSize: 11 }}> km/h</span>
            </div>
            <div style={{ position: 'absolute', top: 64, left: 10, ...pill }}>
              <div style={label}>SCORE</div>
              {hud.score}
            </div>
            <div style={{ position: 'absolute', top: 118, left: 10, ...pill }}>
              <div style={label}>PIÈCES</div>
              <span style={{ color: '#ffd23f' }}>●</span> {hud.coins}
            </div>
            <div
              style={{
                position: 'absolute',
            ...
            >
              {[0, 1, 2].map((i) => (
                <CarIcon key={i} alive={i < hud.lives} />
              ))}
            </div>
            <div
              style={{
            ...
            >
              {hud.shield !== 0 && (
                <div
                  style={{
                    ...pill,
                    background: 'rgba(47,134,255,.92)',
                    textAlign: 'center',
                  }}
                >
                  <div style={label}>BOUCLIER</div>
                  {hud.shield === -1 ? 'PRÊT' : hud.shield + 's'}
                </div>
              )}
              {hud.punches > 0 && (
                <div
                  style={{
                    ...pill,
                    background: 'rgba(255,122,51,.95)',
                    textAlign: 'center',
                  }}
                >
                  <div style={label}>BOXE</div>
                  {hud.punches} coups
                </div>
              )}
              {hud.shrink > 0 && (
                <div
                  style={{
                    ...pill,
                    background: 'rgba(46,196,95,.95)',
                    textAlign: 'center',
                  }}
                >
                  <div style={label}>RÉTRÉCI</div>
                  {hud.shrink}s
                </div>
              )}
              {hud.siren !== 0 && (
                <div
                  style={{
                    ...pill,
                    background: 'rgba(225,29,42,.95)',
                    textAlign: 'center',
                  }}
                >
                  <div style={label}>SIRÈNE</div>
                  {hud.siren === -1 ? 'PRÊTE (↓)' : hud.siren + 's'}
                </div>
              )}
            </div>
          </>
        ...
        {status === 'garage' && (
          ...
          >
            ...
            <div style={{ fontSize: 12, opacity: 0.8 }}>
              Vies : {g.current ? g.current.lives : 3}/3
            </div>
            ...
            <div style={{ alignSelf: 'stretch', marginTop: 2 }}>
              ...
              >
                {SHAPES.map((sh) => {
                  const owned = save.owned.includes(sh.id),
                    equipped = save.equipped === sh.id,
                    afford = save.coins >= sh.price;
                  return (
                    <button
                      key={sh.id}
                      disabled={!owned && !afford}
                      onClick={() =>
                        owned ? equipShape(sh.id) : buyShape(sh.id)
                      }
                      style={{
                        display: 'flex',
                        flexDirection: 'column',
                        ...
                        color: '#fff',
                        cursor: owned || afford ? 'pointer' : 'default',
                        opacity: !owned && !afford ? 0.4 : 1,
                      }}
                    >
                      <ShapePreview
                        shape={sh.id}
            ...
            <div style={{ alignSelf: 'stretch' }}>
              <div style={{ ...label, fontSize: 11, margin: '4px 0 6px' }}>
                COULEUR (gratuite)
              </div>
            ...
            <div style={{ alignSelf: 'stretch' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {PROTECTIONS.map((pr) => {
                  ...
                  const dis =
                    !afford ||
                    (pr.id === 'repair' && g.current && g.current.lives >= 3);
                  return (
                    <button
                      key={pr.id}
                      disabled={dis}
                      onClick={() => buyProtection(pr.id)}
                      style={{
                        display: 'flex',
                    ...
                    >
                      <span>{pr.name}</span>
                      <span style={{ fontWeight: 800, color: '#ffd23f' }}>
                        ● {pr.price}
                      </span>
                    </button>
                  );
            ...
            <button style={{ ...btn, marginTop: 4 }} onClick={resumeRun}>
              Reprendre la route →
            </button>
          </div>
        )}
        ...
        {status === 'menu' && (
          <div style={overlay}>
            ...
            >
              Évite les voitures qui foncent vers toi.
              <br />
              <b style={{ color: '#7fc0ff' }}>Bouclier</b> : protège et casse
              les voitures.
              <b style={{ color: '#ffb088' }}> Gant</b> (<b>↑</b>) : vise la
              distance !<b style={{ color: '#7ee29a' }}> Rétréci</b> : 2× plus
              petit.
              <b style={{ color: '#ff8d8d' }}> Sirène</b> (<b>↓</b>) : ouvre ta
              voie 8 s.
              <br />
              <b style={{ color: '#ff5a5a' }}>⚠ Voitures noires</b> :
              indestructibles, esquive-les !
            </p>
            <div style={{ fontSize: 13, opacity: 0.9 }}>
              <span style={{ color: '#ffd23f' }}>●</span> {save.coins} pièces ·
              Meilleur : {save.best}
            </div>
            <div
              style={{
            ...
            >
              <button style={btn} onClick={() => setMode('solo')}>
                🚗 Partie solo
              </button>
              <button
                style={{ ...btn, background: '#ff8d3f' }}
                onClick={() => {
                  setDuelNames([save.name || '', '']);
                  setMode('duelSetup');
                }}
              >
                ⚔️ Duel à 2 joueurs
        ...
        {status === 'solo' && (
          <div style={overlay}>
            ...
            <div style={{ fontSize: 12, opacity: 0.7 }}>
              ← → bouger · ↑ coup de poing · ↓ sirène
            </div>
            <button style={btn2} onClick={() => setMode('menu')}>
              ← Menu
            </button>
          </div>
        )}
        ...
        {status === 'duelSetup' && (
          <div style={overlay}>
            ...
            >
              Chacun joue à tour de rôle. Le gagnant est celui qui cumule le
              plus de points !
            </div>
            <input
              value={duelNames[0]}
              onChange={(e) => setDuelNames([e.target.value, duelNames[1]])}
              maxLength={14}
            ...
            <input
              value={duelNames[1]}
              onChange={(e) => setDuelNames([duelNames[0], e.target.value])}
              maxLength={14}
              ...
              style={{
                ...
                padding: '10px 12px',
                borderRadius: 10,
                border: '2px solid rgba(255,141,63,.5)',
                background: 'rgba(255,255,255,.1)',
                color: '#fff',
                fontSize: 15,
                textAlign: 'center',
            ...
            <button
              style={{
                ...btn,
                opacity: duelNames[0].trim() && duelNames[1].trim() ? 1 : 0.5,
              }}
              disabled={!(duelNames[0].trim() && duelNames[1].trim())}
              onClick={beginDuel}
            >
              Commencer le duel →
            </button>
        ...
        {status === 'turnIntro' && tour && tour.mode === 'duel' && (
          <div style={overlay}>
            <div style={{ fontSize: 14, opacity: 0.8, letterSpacing: 1 }}>
              MANCHE {tour.scores[tour.turn].length + 1} / {tour.gamesPer}
            </div>
            ...
            <div
              style={{
                fontSize: 30,
                fontWeight: 900,
                color: tour.turn === 0 ? '#ffd23f' : '#ff8d3f',
              }}
            >
              {tour.names[tour.turn]}
            </div>
            {tour.scores[0].length + tour.scores[1].length > 0 && (
              <div style={{ fontSize: 13, opacity: 0.8 }}>
                Cumul — {tour.names[0]} : <b>{duelTotals(tour)[0]}</b> ·{' '}
                {tour.names[1]} : <b>{duelTotals(tour)[1]}</b>
              </div>
            )}
            <button style={btn} onClick={startTurn}>
              Prêt ! →
            </button>
          </div>
        )}
        ...
        {status === 'over' && tour && tour.mode === 'solo' && (
          <div style={overlay}>
            ...
            <div style={{ fontSize: 14, opacity: 0.9 }}>
              <span style={{ color: '#ffd23f' }}>●</span> {save.coins} pièces en
              banque
            </div>
            <button style={btn} onClick={launchGame}>
              Rejouer
            </button>
            <button style={btn2} onClick={() => setMode('menu')}>
              ← Menu
        ...
        {status === 'over' && tour && tour.mode === 'duel' && (
          <div style={overlay}>
            ...
            <div style={{ fontSize: 22 }}>
              Score de la manche : <b>{lastScore}</b>
            </div>
            <button style={btn} onClick={afterDuelGame}>
              Continuer →
            </button>
          </div>
        )}
        ...
        {status === 'standings' && tour && tour.mode === 'duel' && (
          ...
          >
            ...
            <div style={{ fontSize: 13, opacity: 0.8 }}>
              après {tour.scores[1].length} manche
              {tour.scores[1].length > 1 ? 's' : ''} sur {tour.gamesPer}
            </div>
            {DuelTable(tour)}
            {(() => {
              const tt = duelTotals(tour);
              const d = Math.abs(tt[0] - tt[1]);
              const ld = tt[0] === tt[1] ? null : tt[0] > tt[1] ? 0 : 1;
              return (
                <div style={{ fontSize: 14, opacity: 0.92 }}>
                  {ld == null ? (
                    ...
                    <span>
                      <b style={{ color: ld === 0 ? '#ffd23f' : '#ff8d3f' }}>
                        {tour.names[ld]}
                      </b>{' '}
                      mène de {d} point{d > 1 ? 's' : ''}
                    </span>
                  )}
                </div>
              );
            })()}
            <button style={btn} onClick={() => setMode('turnIntro')}>
              Continuer le duel →
            </button>
          </div>
        )}
        ...
        {status === 'duelEnd' && tour && tour.mode === 'duel' && (
          ...
          >
            ...
            {DuelTable(tour)}
            <button style={btn} onClick={() => setMode('duelSetup')}>
              Rejouer un duel
            </button>
            <button style={btn2} onClick={() => setMode('menu')}>
              ← Menu
      ...
      <div style={{ display: 'flex', gap: 8, width: '100%', maxWidth: W }}>
        <button
          style={tBtn}
        ...
        <button
          style={{ ...tBtn, flex: 0.7, background: 'rgba(226,59,59,.9)' }}
          onPointerDown={pressPunch}
        >
          <GloveBtnIcon />
        </button>
        <button
          style={{ ...tBtn, flex: 0.7, background: '#fff' }}
          onPointerDown={pressSiren}
        >
          <SirenBtnIcon />
        </button>
        <button
